...
export function createWorkspaceCanvas(options: WorkspaceCanvasOptions) {
    ...
    function computeImagePositionNextToThread(
        threadNode: AiChatThreadCanvasNode,
        threadId: string,
        imageWidth: number,
        imageHeight: number
    ): { x: number; y: number } {
        // Collect existing images generated by this specific thread
        const existingGeneratedImages = (currentCanvasState?.nodes || []).filter(
            (n: CanvasNode): n is ImageCanvasNode =>
                n.type === 'image' && n.generatedBy?.aiChatThreadId === threadId
        )

        // Also count any in-progress partial for this thread (not yet in canvasState as generatedBy)
        // Note: The active partial is the one being positioned, so we don't add it to the count
        // effectively using existingCount as the index (0-based) for the new item.
        const existingCount = existingGeneratedImages.length

        // Stack images vertically to the right of the thread with 30px gap between them
        const gapX = 50
        const gapY = 30
        const x = threadNode.position.x + threadNode.dimensions.width + gapX
        // Use existingCount as index: 0 for first image, 1 for second, etc.
        const y = threadNode.position.y + existingCount * (imageHeight + gapY)
    ...
    setAiGeneratedImageCallbacks({
        ...
        onImagePartialToCanvas: async (data) => {
            ...
            const position = computeImagePositionNextToThread(sourceThread, threadId, imageWidth, imageHeight)
            ...
            // Persist state but prevent full re-render (would destroy active editors mid-stream)
        ...
        onImageCompleteToCanvas: async (data) => {
            ...
            if (partial) {
                ...
                // Set the edge's sourceMessageId to link to the specific AI response
                ...
                const collisionResult = resolveCollisions(nodeBoxes)
                ...
                // Node structure unchanged (same nodeId:type), so shouldRerender → false.
                // No need for commitCanvasStatePreservingEditors here.
                commitCanvasState({
                    viewport: currentCanvasState?.viewport || { x: 0, y: 0, zoom: 1 },
                    nodes: resolvedNodes,
                    edges,
                ...
                const allNodes = [...existingNodes, imageNode]
                ...
                // Use the resolved position for the DOM node
                ...
                // Persist but prevent full re-render that would destroy active editors
