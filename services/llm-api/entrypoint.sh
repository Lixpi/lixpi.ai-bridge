...
env | grep -E "(NATS_|OPENAI_|ANTHROPIC_|AUTH0_)" | sed 's/\(API_KEY\)=.*/\1=***REDACTED***/' | sort
...
if [ -z "$OPENAI_API_KEY" ] && [ -z "$ANTHROPIC_API_KEY" ]; then
    echo "⚠️  WARNING: No AI provider API keys found. At least one of OPENAI_API_KEY or ANTHROPIC_API_KEY should be set."
fi
